...
;;;	recover-from-disk re-run-story tdebug say-all print-params 
;;;     reset-params init-tspin tspin-output-var
;;; 
...
;;; (re-run-story '<REF#>)". This feature is useful in order to track down bugs
;;; (especially intermittent one), or to run a previously established sequence
;;; (for demonstration purposes perhaps).
;;;
...
;;;
(defun init-random-seed (&optional from-file? (file-name "seed.file"))
...
;;;
;;; Function print-story-params is called by function spin to print a
;;; message identifying the story id before the story is output. It also
;;; lists the call necessary to recreate the story at as later time. An
;;; important side-effect is that the function also saves the story
;;; information locally and to disk so that it can be rerun and if a
;;; crash occurs, the story sequence can be recovered.
;;; 
(defun print-story-params (alt? person-num problem-num)
  (let* ((function-call 
    ...
    (format
      *tspin-stream*
      "~%RECREATE STORY ~s BY (re-run-story '~s)~%" 
      story-id 
      story-id))
  )
...
(defun specify-yarns (person-num problem-num &optional control-num)
  ...
  (if (and (null person-num)
	...
	(4 (setf person-num 4)(setf problem-num 0))
	(5 (setf person-num 5)(setf problem-num 1))
	(6 (setf person-num 5)(setf problem-num 0)
	   (setf *force-original-demo* t))
	(7 (make-wild-story)
	   (setf *auto?* t))
	(8 (make-wild-story nil)
	   (setf *auto?* t))
	(9 (setf *allow-random-events* t)
	   (setf *request-for-random-event* t)
	   (setf *random-odds* 0)
	   )
	(10 (setf *annoy-dog-freq* 100)
	    (setf *high-4-entertainment* 0)
	    (setf person-num 5)(setf problem-num 3)
	    )
	(11 (setf person-num 3)(setf problem-num 1))
	(12 (setf *auto?* nil))
	(13 (user-control-prog-parameters)
	    (setf *auto?* nil))
	))
...
;  (in-package 'TSPIN)
  ...
  (init-file)				; Initialize crash recovery file.
  (init-world)
  )
