...
(defvar *comply-freq*  90)            ;; chance of suspect handing over contraband if asked
...
(defun mspin (&optional alt? person-num problem-num)
  ...
  (spin alt? person-num problem-num))
...
(defun pick-one (l)
  ...
  (let ((len (length l)))
    (if *auto?*
	 ...
	 (clear-input)
	 (let ((x 
                (read) ;Read from *standard-io* rather than *terminal-io*
;                (read t) 
                ))
